...
- Added a new path tracking task (``mjlab.tasks.path_tracking``) with NUbots
  ...
  horizon of upcoming walk-path poses expressed relative to the robot's own
  frame — the same interface the deployed walk path planner can provide from
  odometry, so no observation is used in training that is unavailable on the
  real robot. Paths are generated by integrating sampled twist segments from
  the robot's pose and are regenerated mid-episode to emulate on-the-fly
