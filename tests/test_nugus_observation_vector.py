...
def test_history_group_clones_actor_terms(nugus_flat_cfg) -> None:
  ...
  """
  history = nugus_flat_cfg.observations["history"]
  ...
  assert list(history.terms.keys()) == list(
    nugus_flat_cfg.observations["actor"].terms.keys()
  )
...
def test_actor_uses_history_model() -> None:
