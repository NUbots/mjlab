...
def gait_clock(
  ...
  ``phase_source="policy"`` reads the phase owned by the ``phase_delta`` action
  term instead of episode time, which is the ``clock_owned`` configuration: the
  policy advances its own clock and the observation reports where it put it.
